...
typeof g.length||1>g.length)g=Array(g);if(g.length)for(var j=0;j<g.length;j++){var l=g[j].url,m=g[j].str,k=g[j].icon,h=g[j].target,w="",o="";k&&(w=" style=\"background-image:url('"+k+"')\" ");switch(h){case "popup":o=" onclick=\"popopen(this.href,'"+h+"'); return false;\"";break;case "javascript":o=' onclick="'+l+'; return false; "';l="#";break;default:o=' onclick="window.open(this.href); return false;"'}b+="<li "+w+'><a href="'+l+'"'+o+">"+m+"</a></li> "}}this.loaded_popup_menus[c]=b}if(b)c=a("#popup_menu_area").html("<ul>"+
...
a.length;)d=this._keyStr.indexOf(a.charAt(l++)),c=this._keyStr.indexOf(a.charAt(l++)),g=this._keyStr.indexOf(a.charAt(l++)),j=this._keyStr.indexOf(a.charAt(l++)),d=d<<2|c>>4,c=(c&15)<<4|g>>2,e=(g&3)<<6|j,b+=String.fromCharCode(d),64!=g&&(b+=String.fromCharCode(c)),64!=j&&(b+=String.fromCharCode(e));return b=Base64._utf8_decode(b)},_utf8_encode:function(a){for(var a=a.replace(/\r\n/g,"\n"),b="",d=0;d<a.length;d++){var c=a.charCodeAt(d);128>c?b+=String.fromCharCode(c):(127<c&&2048>c?b+=String.fromCharCode(c>>
...
function xml2json(a,b,d){var c={toObj:function(a){var b={};if(1==a.nodeType){if(d&&a.attributes.length)for(var e=0;e<a.attributes.length;e++)b["@"+a.attributes[e].nodeName]=(a.attributes[e].nodeValue||"").toString();if(a.firstChild){for(var m=e=0,k=!1,h=a.firstChild;h;h=h.nextSibling)1==h.nodeType?k=!0:3==h.nodeType&&h.nodeValue.match(/[^ \f\n\r\t\v]/)?e++:4==h.nodeType&&m++;if(k)if(2>e&&2>m){c.removeWhite(a);for(h=a.firstChild;h;h=h.nextSibling)3==h.nodeType?b=c.escape(h.nodeValue):4==h.nodeType?
...
(b=null)}else 9==a.nodeType?b=c.toObj(a.documentElement):alert("unhandled node type: "+a.nodeType);return b},toJson:function(a,b,d){var e=b?'"'+b+'"':"";if(a instanceof Array){for(var k=0,h=a.length;k<h;k++)a[k]=c.toJson(a[k],"",d+"\t");e+=(b?":[":"[")+(1<a.length?"\n"+d+"\t"+a.join(",\n"+d+"\t")+"\n"+d:a.join(""))+"]"}else if(null==a)e+=(b&&":")+"null";else if("object"==typeof a){k=[];for(h in a)k[k.length]=c.toJson(a[h],h,d+"\t");e+=(b?":{":"{")+(1<k.length?"\n"+d+"\t"+k.join(",\n"+d+"\t")+"\n"+
d:k.join(""))+"}"}else e="string"==typeof a?e+((b&&":")+'"'+a.toString()+'"'):e+((b&&":")+a.toString());return e},innerXml:function(a){var b="";if("innerHTML"in a)b=a.innerHTML;else for(var c=function(a){var b="";if(1==a.nodeType){for(var b=b+("<"+a.nodeName),d=0;d<a.attributes.length;d++)b+=" "+a.attributes[d].nodeName+'="'+(a.attributes[d].nodeValue||"").toString()+'"';if(a.firstChild){b+=">";for(d=a.firstChild;d;d=d.nextSibling)b+=c(d);b+="</"+a.nodeName+">"}else b+="/>"}else 3==a.nodeType?b+=
...
m.elements.ruleset.value;else if(k._filter)p=m.elements._filter.value;if(!p)return!0;a.isFunction(j[p])&&(A=j[p]);p=a.extend({},g[p.toLowerCase()]||{},l);s=[];for(q=0,t=m.elements.length;q<t;q++)n=k[q],(r=n.name)&&k[r]&&(!k[r].length||k[r][0]===n)&&s.push(r);s=s.join("\n");u={};for(r in p)if(p.hasOwnProperty(r)&&(n=[],"^"==r.substr(0,1))){(n=s.match(RegExp("^"+o(r.substr(1))+".*$","gm")))||(n=[]);for(q=0,t=n.length;q<t;q++)u[n[q]]=p[r];p[r]=null;delete p[r]}p=a.extend(p,u);for(r in p)if(p.hasOwnProperty(r)&&
(f=p[r],s=(n=k[r])?a.trim(b(a(n))):"",u=(f.modifier||"")+",",n&&!n.disabled)){if(f["if"]){a.isArray(f["if"])||(f["if"]=[f["if"]]);for(q=0;q<f["if"].length;q++)if(t=f["if"][q],n=new Function("el","return !!("+t.test.replace(/\$(\w+)/g,'el["$1"].value')+")"),n(k))f[t.attr]=t.value}if(s){n=parseInt(f.minlength)||0;t=parseInt(f.maxlength)||0;z=/b$/.test(f.minlength||"");y=/b$/.test(f.maxlength||"");q=s.length;if(z||y)B=d(s);if(n&&n>(z?B:q)||t&&t<(y?B:q))return this.cast("ALERT",[m,r,"outofrange",n,t])&&
!1;if(f.equalto&&(t=(q=k[f.equalto])?a.trim(b(a(q))):"",q&&t!==s))return this.cast("ALERT",[m,r,"equalto"])&&!1;y=(f.rule||"").split(",");for(q=0,t=y.length;q<t;q++)if(z=y[q])if(n=this.cast("APPLY_RULE",[z,s]),-1<u.indexOf("not,")&&(n=!n),!n)return this.cast("ALERT",[m,r,"invalid_"+z])&&!1}else if(f["default"]&&(s=f["default"]),f.required)return this.cast("ALERT",[m,r,"isnull"])&&!1}return a.isFunction(A)?A(m):!0},API_ADD_RULE:function(a,b){var c=b[0].toLowerCase();e[c]=b[1]},API_DEL_RULE:function(a,
...
b){alert(b[0])},API_ADD_MESSAGE:function(a,b){c[b[0]]=b[1]},API_GET_MESSAGE:function(a,b){var d=b[0];return c[d]||d},API_ADD_CALLBACK:function(a,b){j[b[0]]=b[1]},API_REMOVE_CALLBACK:function(a,b){delete j[b[0]]}}));xe.registerApp(m);var k=xe.createPlugin("editor_stub",{API_BEFORE_VALIDATE:function(a,b){var c=b[0].getAttribute("editor_sequence");if(c&&"object"==typeof c)c=c.value;if(c)try{editorRelKeys[c].content.value=editorRelKeys[c].func(c)||""}catch(d){}}});m.registerPlugin(new k)})(jQuery);
