...
class Install extends Base
{
	...
	/**
	 * When clearing cache, delete event handlers associated with deleted modules.
	 */
	...
	/**
	 * Migrate old sites and multidomain info to new 'domains' table.
	 */
	public function migrateDomains()
	{
		...
		$config = ModuleConfigModel::getModuleConfig('module');
		...
		if ($output->data)
		{
			foreach ($output->data as $site_info)
			{
				...
				$domain->domain = URL::getDomainFromURL(strtolower($site_domain));
		...
		else
		{
			...
			$default_hostinfo = parse_url(URL::getCurrentURL());

			$domain = new \stdClass();
			$domain->domain_srl = 0;
			$domain->domain = URL::decodeIdna(strtolower($default_hostinfo['host']));
			$domain->is_default_domain = 'Y';
		...
		return DomainModel::getDomain($default_domain->domain_srl);
	}
}
