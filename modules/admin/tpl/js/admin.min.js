...
jQuery(function(a){a.fn.xeMenu=function(){this.attr("role","navigation").find(">.nav>li").attr("role","menuitem").find(">ul").css("height","0").end().filter(":has(>ul)").attr("aria-haspopup","true").end().end().find(">.nav").mouseover(function(){a(this).parent(".gnb").addClass("active").end().find(">li>ul").css("height","auto").end()}).mouseleave(function(){a(this).parent(".gnb").removeClass("active").end().find(">li>ul").css("height","0").end()}).focusout(function(){var d=a(this);setTimeout(function(){d.find(":focus").length||
d.mouseleave()},1)}).delegate("a",{focus:function(){a(this).mouseover()}});this.find(">.bmk").removeClass("active").mouseover(function(){a(this).addClass("active")}).mouseleave(function(){a(this).removeClass("active")}).focusout(function(){var d=a(this);setTimeout(function(){d.find(":focus").length||d.mouseleave()},1)}).delegate("a",{focus:function(){a(this).mouseover()}})};a("div.gnb").xeMenu();a(".gnb>.mnv").change(function(){window.location.href=a(this).find("option:selected").val()})});
...
"click",function(b){var f,h;f=a(this);if(h=f.data("moduleInstanceList")){e.sSelectedModuleType=f.text();m.empty();for(var l in h)h.hasOwnProperty(l)&&($li=a("<li />").appendTo(m),a('<a href="#" >').html("<div>"+h[l].browser_title+"</div>").data("module_srl",h[l].module_srl).appendTo($li));n.show();d(m,x);o.find("li").removeClass("on");f.parent("li").addClass("on");b.preventDefault()}}).end().find(".moduleInstanceListUL").delegate("a","click",function(b){$this=a(this);e.sSelectedModuleInstanceName=
$this.text();e.sSelectedModuleSrl=$this.data("module_srl");j.trigger("moduleSelect",[e.sSelectedModuleType,e.sSelectedModuleInstanceName,e.sSelectedModuleSrl]);a(".tgAnchor.moduleSearch").trigger("close.tc");b.preventDefault()});return this};a(".moduleSearch").each(a.fn.xeModuleSearch)});
...
d);f=e.find(".langList");a.each(d,function(b){var d=a("<li />").appendTo(f),g=e.data("layer_index");a('<a href="#langInput_'+g+'" class="langItem" />').text(this[xe.current_lang]).data("multilang-name",b).appendTo(d)});var b=d,d=0,k;for(k in b)b.hasOwnProperty(k)&&d++;d>1&&e.addClass("showChild");e.find(".langList>li>a:first").click()}});show_waiting_message=!0}}).delegate("button._btnLang",{click:function(){var b=a(this);g=!0;k.trigger("hide");b.closest(".multiLangEdit").find("input.vLang,textarea.vLang").eq(0).val(b.data("langkey")).end().eq(1).val(b.text()).end();
return!1},mousedown:function(){f=!0},focus:function(){a(this).mouseover()},mouseover:function(){a(this).closest("ul").find("button.active").removeClass("active")}});k=a('<div class="suggestion"><ul></ul></div>').bind("show",function(){a(this).show()}).bind("hide",function(){a(this).hide()});var n=0});
