...
1).addClass("x_active").show().siblings().removeClass("x_active").hide();a(this).parents(".x_tabbable").trigger("tab_change",[parseInt(a(this).attr("data-index"))-1,a(this)]);return!1});a.fn.gnb=function(){var b=a(".x>.xin>.body"),d=b.find(">.gnb"),e=d.find(">ul>li"),g=new Date;g.setTime(g.getTime()+30758400);e.find(">a").prepend("<i />");e.each(function(b){a(this).attr("data-index",b+1)});var k=e.find(">ul>li.active_").closest("li.active").attr("data-index");e.find(">ul>li.active_").clone().addClass("active").attr("data-index",
k).prependTo("#gnbNav").find(">a").prepend("<i />");e.find("ul").prev("a").bind("click focus",function(){a(this).parent("li").addClass("open").siblings("li").removeClass("open");b.removeClass("wide");f();return!1});d.find('>a[href="#gnbNav"]').click(function(){a(this).parent(".gnb").toggleClass("open");b.toggleClass("wide");980>=a(window).width()&&!d.hasClass("open")&&a("#gnbNav").removeClass("ex");f();a(this).parent(".gnb").hasClass("open")?setCookie("__xe_admin_gnb_status","open",g):setCookie("__xe_admin_gnb_status",
"close",g);return!1});d.prepend('<button type="button" class="close before" />').append('<button type="button" class="close after" />');d.find(">.close").focus(function(){b.addClass("wide");f()});d.find(".exMenu>button").click(function(){a("#gnbNav").toggleClass("ex");f();a("#gnbNav").hasClass("ex")?setCookie("__xe_admin_gnb_ex_status","open",g):setCookie("__xe_admin_gnb_ex_status","close",g)});(e=getCookie("__xe_admin_gnb_status"))&&setCookie("__xe_admin_gnb_status",e,g);(e=getCookie("__xe_admin_gnb_ex_status"))&&
setCookie("__xe_admin_gnb_xe_status",e,g)};a(".gnb").gnb();a(".x #lang").mouseleave(function(){a(this).hide()}).focusout(function(){var b=a(this);setTimeout(function(){b.find("a:focus").length||b.mouseleave()},500)});a('.x th>input[type="checkbox"]').change(function(){var b=a(this),d=b.data("name");b.closest("table").find("input:checkbox").filter(function(){var b=a(this);return!b.prop("disabled")&&(b.attr("name")==d||b.data("name")==d)}).prop("checked",b.prop("checked")).end().end().trigger("update.checkbox",
[d,this.checked])});a(document.body).on("click",".x .x_pagination .x_disabled, .x .x_pagination .x_active",function(){return!1});if(1<a(".section").length){var e=a(".x .section").find(">h1:first");e.append('<button type="button" class="snToggle x_icon-chevron-up">Toggle this section</button>');a(".x .section.collapse>h1>.snToggle").removeClass("x_icon-chevron-up").addClass("x_icon-chevron-down");e.find(">.snToggle").click(function(){var b=a(this),d=b.closest(".section");d.hasClass("collapse")?(d.removeClass("collapse").children("h1:first").siblings().show(),
b.removeClass("x_icon-chevron-down").addClass("x_icon-chevron-up")):(d.addClass("collapse").children("h1:first").siblings().hide(),b.removeClass("x_icon-chevron-up").addClass("x_icon-chevron-down"));f()})}e=a(".x .x_alert");e.prepend('<button type="button" class="x_close">&times;</button>');e.children(".x_close").click(function(){a(this).parent(".x_alert").hide()});a(".x .x_btn").click(function(){if(a(this).hasClass("x_disabled"))return!1});a.fn.vr=function(){this.each(function(){var b=a(this);"|"==
b.text()&&b.addClass("vr").filter(":first-child, :last-child").remove()})};a(".x i").vr();a.fn.labelMaker=function(){this.each(function(b){var b=b+1,d=a(this),b="i"+b;d.next("input, textarea, select").filter("[id]").not(":radio, :checkbox").length?d.attr("for",d.next().attr("id")):d.next("input, textarea, select").not("[id]").not(":radio, :checkbox").length?d.attr("for",b).next().attr("id",b):d.prev(":radio, :checkbox").filter("[id]").length?d.attr("for",d.prev().attr("id")):d.prev(":radio, :checkbox").not("[id]").length?
...
b.html(d.replace(/(@.+)$/,'<span class="ellipsis">...</span><span class="cover">$1</span>')).find(">.ellipsis").css({position:"absolute",zIndex:1}).hover(function(){a(this).next(".cover").mouseover()},function(){a(this).next(".cover").mouseout()}).end().find(">.cover").css({zIndex:2,opacity:0}).hover(function(){a(this).css("opacity",1).prev("span").css("visibility","hidden")},function(){a(this).css("opacity",0).prev("span").css("visibility","visible")}).end()})};a(".masked").xeMask();a.fn.tableSpan=
function(){this.each(function(){var b=a(this),d=b.find(">thead>tr:eq(0)>th").length,b=b.find(">tbody>tr:eq(0)>td:only-child");d!=b.attr("colspan")&&b.attr("colspan",d).css("text-align","center")})};a("table").tableSpan()});
...
jQuery(function(a){a.fn.xeModuleFinder=function(){this.not(".xe-module-finder").addClass("xe-module-finder").find("a.tgAnchor.findsite").bind("before-open.tc",function(){var f,e,b;f=a(this);e=a(f.attr("href")).find(">ul");b=f.prev("input:text").val();a.exec_json("admin.getSiteAllList",{domain:b},function(b){var j=b.site_list,g,k;e.empty();f.closest(".modulefinder").find(".moduleList,.moduleIdList").attr("disabled","disabled");if(b.error||!a.isArray(j))f.trigger("close.tc");else{g=0;for(k=j.length;g<
k;g++)b=a("<li />").appendTo(e),a('<button type="button" />').text(j[g].domain).data("site_srl",j[g].site_srl).appendTo(b)}})}).end().find(".tgContent.suggestion").delegate("button","click",function(){var f,e;f=a(this);e=f.closest(".modulefinder");e.find("a.tgAnchor.findsite").trigger("close.tc");a.exec_json("module.procModuleAdminGetList",{site_srl:f.data("site_srl")},function(b){var d=b.module_list,f;if(!b.error&&d){b=e.find(".moduleList").data("module_list",d).removeAttr("disabled").empty();for(f in d)d.hasOwnProperty(f)&&
...
c;i++){d=g[i].domain;if(b){if(!d.match(e))continue;d=d.replace(e,function(a){return'<span class="highlight">'+a+"</span>"})}$li=a("<li />").appendTo(n);a("<a>").attr("href","#").html(d).data("site_srl",g[i].site_srl).appendTo($li)}f(n,t-q.parent("div").height());1==g.length&&n.find("a").trigger("click")}if(d&&d.html){b.after(d.html).addClass("tgAnchor").attr("href","#"+j).xeContentToggler();var l=b.next(".moduleWindow"),d=l.find(".siteList"),m=l.find(".moduleList"),p=l.find(".instanceList"),n=d.find(">ul"),
s=m.find(">ul"),r=p.find(">select"),q=l.find("input.siteFinder"),u,t=280;q.keyup(function(){h(q.val())});"undefined"==typeof console&&(console={log:function(){}});b.not(".xe-module-search").addClass("xe-module-search").parent().find(".moduleTrigger").bind("before-open.tc",function(){var b;b=a(this);n.empty();r.empty();m.hide();p.hide();a.exec_json("admin.getSiteAllList",{domain:""},function(d){var e=d.site_list;d.error||!a.isArray(e)?b.trigger("close.tc"):(u=e,h(q.val()),q.focus())})});l.find(".siteList>ul").delegate("a",
"click",function(b){var d;d=a(this);d.closest(".moduleSearch");s.empty();p.hide();a.exec_json("module.procModuleAdminGetList",{site_srl:d.data("site_srl")},function(b){var e=b.module_list,g;if(!b.error&&e){for(g in e)e.hasOwnProperty(g)&&($li=a("<li />").appendTo(s),a("<a>").attr("href","#").html(e[g].title).data("moduleInstanceList",e[g].list).appendTo($li));l.find(".moduleList").show();f(s,t);n.find("li").removeClass("x_active");d.parent("li").addClass("x_active")}});b.preventDefault()}).end().find(".moduleList>ul").delegate("a",
"click",function(b){var d,g;d=a(this);if(g=d.data("moduleInstanceList")){e.sSelectedModuleType=d.text();r.empty();for(var h in g)g.hasOwnProperty(h)&&($li=a("<option />").html(g[h].browser_title+" ("+g[h].mid+")").appendTo(r).val(g[h].module_srl).data("mid",g[h].mid).data("module_srl",g[h].module_srl).data("layout_srl",g[h].layout_srl).data("browser_title",g[h].browser_title));p.show();f(r,t);s.find("li").removeClass("x_active");d.parent("li").addClass("x_active");b.preventDefault()}}).end().find(".moduleSearch_ok").click(function(d){var f=
...
jQuery(function(a){var f=a('<tr class="placeholder"><td>&nbsp;</td></tr>');a.fn.xeSortableTable=function(){this.not(".xe-sortable-table").addClass("xe-sortable-table").delegate("button.dragBtn","mousedown.st",function(e){var b,d,j,g,k,h,l,m,p,n;if(1==e.which){d=a(this);j=d.closest("tr");g=d.closest("table");n=g.get(0).offsetParent;d=j.height();k=j.width();before_event=a.Event("before-drag.st");g.trigger(before_event);if(before_event.isDefaultPrevented())return!1;b=e.pageY;h=getOffset(j.get(0),n);
$clone=j.attr("target",!0).clone(!0).appendTo(g);p=(e=g.find("thead th")).length;e.filter("[colspan]").attr("colspan",function(a,b){p+=b-1});f.find("td").attr("colspan",p);l=[];g.find("tbody>tr:not([target],.sticky,:hidden)").each(function(){var b=a(this),d;d=getOffset(this,n);l.push({top:d.top,bottom:d.top+b.height(),$item:b})});$clone.addClass("draggable").css({position:"absolute",opacity:0.6,width:k,height:d,left:h.left,top:h.top,zIndex:100});f.css({position:"absolute",opacity:0.6,width:k,height:"10px",
...
l=!1;return!0}function d(a,b){var d=g.data("lang-target");l=!1;d&&d.trigger("selected.g11n",[a,b])}function j(b,g,n,k,l){"undefined"==typeof b&&(b=1);"undefined"==typeof g&&(g=xe.current_lang);"undefined"==typeof n&&(n="");"undefined"==typeof k&&(k="");"undefined"==typeof l&&(l=!0);a.exec_json("module.getModuleAdminLangListHtml",{page:b,lang_code:g,search_keyword:n,name:k,list_count:f.list_count},function(b){b&&b.html&&(h.html(b.html),h.find(".x_pagination a").click(function(){var b=a(this).data("page"),
...
f,function(f){f&&(!f.error&&f.name)&&(d.each(function(){var b=a(this);b.data("value",b.data("tmp_value"))}),e.children("span").html(g),h.find(".cancel").trigger("click"),b.find(".useit").trigger("click"))});return!1}),h.find(".useit").click(function(){var b=a(this).closest(".item").find('[href^="#lang-"]'),e=b.data("lang_code"),b=b.children("span").text();d(e,b)}),l&&(document.location.href="#lang_search"),k&&a("#lang_search").find('[href^="#lang-"]').trigger("click"))})}var g=a(this),k=g.find("#lang_create"),
h=g.find("#lang_search"),l=!1,f=a.extend({create_type:"save_and_use",modify_type:"save_and_use",view_use:!0,view_modify:!0,view_delete:!1,list_count:5},f||{});"save_and_use"!=f.create_type&&k.find(".save-useit").text(xe.cmd_save);g.find(".x_tabbable").xeTabbable();k.find(".editMode textarea").change(function(){l=!0});k.submit(function(){var e={},g=null;a(this).find(".editMode").children("textarea").each(function(){var b=a(this);e[b.attr("class")]=b.val();xe.current_lang==b.attr("class")&&(g=b.val())});
...
k.is(":visible")&&l&&confirm(xe.msg_confirm_save_and_use_multilingual)&&k.find(".save-useit").trigger("click");a=!e()||!b()?!1:void 0;return a});return this}});var is_loaded_multilingual_window_html=!1;jQuery(function(a){a.fn.xeLoadMultilingualWindowHtml=function(){a.exec_json("module.getModuleAdminMultilingualHtml",{},function(f){var e=a(".x #content");a(f.html).appendTo(e).xeMultilingualWindow();a(".lang_code").trigger("loaded-multilingualWindow")});return this}});var multilingual_id_count=0;
jQuery(function(a){a.fn.xeApplyMultilingualUI=function(){a(this).each(function(){function f(){function f(){var b=p.val();l.trigger("click");p.val(b);h.val(b);/^\$user_lang->/.test(h.val())&&a.exec_json("module.getModuleAdminLangCode",{name:h.val().replace("$user_lang->","")},function(a){if(a&&a.langs){var b=h.width();h.closest(".g11n").addClass("active");h.val(a.langs[xe.current_lang]).attr("disabled","disabled").width(b-44)}})}var g=a("#g11n"),k=e.width()-44,h="TEXTAREA"==b.tagName||"textarea"==
e.data("type")?a('<textarea id="lang_'+d+'" class="displayInput" style="width:'+k+'px">').data("lang-id",d):a('<input type="text" id="lang_'+d+'" class="displayInput" style="width:'+k+'px">').data("lang-id",d),l=a('<button type="button" class="x_add-on remover" title="'+xe.cmd_remove_multilingual_text+'"><i class="x_icon-remove"></i>'+xe.cmd_remove_multilingual_text+"</button>").data("lang-target",d),m=a('<a href="#g11n" class="x_add-on modalAnchor" title="'+xe.cmd_set_multilingual_text+'"><i class="x_icon-globe"></i>'+
xe.cmd_set_multilingual_text+"</a>").data("lang-target",d);e.parent().addClass("g11n").addClass("x_input-append");e.after(h,l,m);e.hide();m.attr("href","#g11n").xeModalWindow();h.bind("selected.g11n",function(b,d,e){b=h.width();a(this).parent().hasClass("active")||(b-=44);h.width(b).attr("disabled","disabled").val(e).parent(".g11n").addClass("active");h.siblings("#"+h.data("lang-id")).val("$user_lang->"+d);m.trigger("close.mw")});m.bind("open.mw",function(){var b=a(this),d=b.siblings(".displayInput");
