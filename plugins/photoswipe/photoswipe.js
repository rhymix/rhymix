/**
 * PhotoSwipe plugin frontend
 *
 * Based on http://photoswipe.com/documentation/getting-started.html
 * Modified for Rhymix by @misol
 * Ported to plugin by @kijin
 */
(function($) {

	// Load plugin configuration.
	const config = JSON.parse(document.getElementById('photoswipe-config').textContent);

	// Only select user content.
	const gallerySelector = '.rhymix_content';

	// Create the .pswp container.
	const pswp = (function() {
		const pswp = $('<div class="pswp" tabindex="-1" role="dialog" aria-hidden="true"></div>');
		const background = $('<div class="pswp__bg"></div>').appendTo(pswp);
		const wrapper = $('<div class="pswp__scroll-wrap"></div>').appendTo(pswp);

		const container = $('<div class="pswp__container"></div>').appendTo(wrapper);
		container.append('<div class="pswp__item"></div>');
		container.append('<div class="pswp__item"></div>');
		container.append('<div class="pswp__item"></div>');

		const ui = $('<div class="pswp__ui pswp__ui--hidden"></div>').appendTo(wrapper);
		const topBar = $('<div class="pswp__top-bar"></div>').appendTo(ui);
		topBar.append('<div class="pswp__counter"></div>');
		topBar.append('<button class="pswp__button pswp__button--close" title="Close"></button>');
		topBar.append('<button class="pswp__button pswp__button--share" title="Share"></button>');
		topBar.append('<button class="pswp__button pswp__button--fs" title="Fullscreen"></button>');
		topBar.append('<button class="pswp__button pswp__button--zoom" title="Zoom"></button>');
		...
		ui.append('<button class="pswp__button pswp__button--arrow--left" title="Prev"></button>');
		ui.append('<button class="pswp__button pswp__button--arrow--right" title="Next"></button>');

		const caption = $('<div class="pswp__caption"><div class="pswp__caption__center" style="text-align:center"></div></div>').appendTo(ui);
		if (config.display_filename != 'Y') {
	...
	// Function to find the closest parent element.
	function closest(el, fn) {
		return el && (fn(el) ? el : closest(el.parentNode, fn));
	}

	// ================ ORIGINAL ADDON SOURCE ================

	// Skip images that have these classes or are children of these elements.
	...
	ps_skip_elements_array.forEach(function(el, i) {
		ps_skip_elements += el + ' img,';
	});
